...
import org.cvn.videoplayer.weight.CircleTransform;
...
public class ImageUtil {
     ...
     * 设置图片长宽比
     * @param view 展示图片的控件
     * @param spanCount 每行显示的图片数量
     * @param space 图片间隔(dp)
     * @param rate 图片宽高比
     */
    public static void setAspectRatio(View view, int spanCount, float rate, float space,Context context){
        ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
        WindowManager manager = ((Activity) context).getWindowManager();
        Display defaultDisplay = manager.getDefaultDisplay();
        DisplayMetrics outMetrics = new DisplayMetrics();
        defaultDisplay.getMetrics(outMetrics);
        space=AppUtil.dip2px(space);
        int width=(int) (outMetrics.widthPixels/spanCount - space);
        layoutParams.width = width;
        layoutParams.height = (int) (width / rate);
        LogUtils.i("[lchunming]ImageUtil space==>"+space);
        LogUtils.i("[lchunming]ImageUtil width==>"+width);
        LogUtils.i("[lchunming]ImageUtil height==>"+width / rate);
